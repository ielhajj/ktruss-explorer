...
namespace Device { namespace Undirected { namespace COOCSRInput {

template < unsigned int CHECK_IF_DELETED, unsigned int RECOUNT_ALL_EDGES >
__global__ void count_triangles_kernel(COOCSRGraph* g, unsigned int k, Info info) {
    ...
    do {
        ...
        if(graphHasDeletedEdges) {
            if(config.recount == ALL) {
                count_triangles_kernel<1,1> <<< numBlocks, numThreadsPerBlock >>> (g_d, k, info);
            } else { // config.recount == AFFECTED
                count_triangles_kernel<1,0> <<< numBlocks, numThreadsPerBlock >>> (g_d, k, info);
            }
        } else {
            if(config.recount == ALL) {
                count_triangles_kernel<0,1> <<< numBlocks, numThreadsPerBlock >>> (g_d, k, info);
            } else { // config.recount == AFFECTED
                count_triangles_kernel<0,0> <<< numBlocks, numThreadsPerBlock >>> (g_d, k, info);
            }
        }
        ...
        syncStopAndPrintElapsed(&iterTimer, "            Mark deleted edges time");
...
void ktruss(COOGraph* graph_d, COOGraph* truss_d, Info info, Config config) {
    ...
    startTimer(&timer);
    COOCSRGraph* g_d = createEmptyCOOCSROnDevice(config.numNodes, config.numEdges);
    coo2coocsrOnDevice(graph_d, g_d);
    syncStopAndPrintElapsed(&timer, "    Convert undirected COO to COOCSR (not optimized)");
