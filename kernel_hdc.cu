...
namespace Host { namespace Directed { namespace COOCSRInput {

template < unsigned int CHECK_IF_DELETED, unsigned int RECOUNT_ALL_EDGES >
void count_triangles(COOCSRGraph* g, Info info) {
...
template < unsigned int CHECK_IF_DELETED >
void mark_indirectly_affected_edges(COOCSRGraph* g, Info info) {
    for(unsigned int e = 0; e < g->numEdges; ++e) {
        unsigned int dst = g->dstIdx[e];
        if(!CHECK_IF_DELETED || dst != DELETED) {
...
void ktruss(COOGraph* gdir, COOGraph* truss, Info info, Config config) {
    ...
    startTimer(&timer);
    COOCSRGraph* g = createEmptyCOOCSR(config.numNodes, config.numEdges/2);
    coo2coocsr(gdir, g);
    stopAndPrintElapsed(&timer, "    Convert directed COO to COOCSR");
