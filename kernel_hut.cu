...
namespace Host { namespace Undirected { namespace TiledCOOCSRInput {

template < unsigned int CHECK_IF_DELETED, unsigned int RECOUNT_ALL_EDGES >
void count_triangles(TiledCOOCSRGraph* g, unsigned int k, Info info) {
            ...
            for(unsigned int xTile = 0; xTile < tilesPerDim; ++xTile) {
                for(unsigned int e = tileEdgesStart; e < tileEdgesEnd; ++e) {
                    if(RECOUNT_ALL_EDGES || info.edgeAffected[e] == DIRECTLY_AFFECTED) {
                        ...
                        if((!CHECK_IF_DELETED || dst != DELETED) && info.numTriangles[e] < k - 2) {
...
void ktruss(COOGraph* graph, COOGraph* truss, Info info, Config config) {
    ...
    startTimer(&timer);
    TiledCOOCSRGraph* g = createEmptyTiledCOOCSR(config.numNodes, config.numTiles, config.numEdges);
    coo2tiledcoocsr(graph, g);
    stopAndPrintElapsed(&timer, "    Convert undirected COO to tiled COOCSR");
