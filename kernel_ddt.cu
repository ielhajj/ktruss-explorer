...
namespace Device { namespace Directed { namespace TiledCOOCSRInput {

template < unsigned int CHECK_IF_DELETED, unsigned int RECOUNT_ALL_EDGES >
__global__ void count_triangles_kernel(TiledCOOCSRGraph* g, Info info) {
    ...
    if(e < g->numEdges) {
        if(RECOUNT_ALL_EDGES || info.edgeAffected[e] == DIRECTLY_AFFECTED || info.edgeAffected[e] == INDIRECTLY_AFFECTED) {
            ...
            if(!CHECK_IF_DELETED || dst != DELETED) {
                ...
                for(unsigned int xTile = src2Tile + blockIdx.y; xTile < tilesPerDim; xTile += gridDim.y) {
...
template < unsigned int CHECK_IF_DELETED >
__global__ void mark_indirectly_affected_edges_kernel(TiledCOOCSRGraph* g, Info info) {
    unsigned int e = blockIdx.x*blockDim.x + threadIdx.x;
    if(e < g->numEdges) {
        unsigned int dst = g->dstIdx[e];
        if(!CHECK_IF_DELETED || dst != DELETED) {
    ...
    do {
        ...
        if(graphHasDeletedEdges) {
            if(config.recount == ALL) {
                count_triangles_kernel<1,1> <<< dim3(numBlocks, config.numParallelTiles), numThreadsPerBlock >>> (g_d, info);
            } else { // config.recount == AFFECTED
                count_triangles_kernel<1,0> <<< dim3(numBlocks, config.numParallelTiles), numThreadsPerBlock >>> (g_d, info);
            }
        } else {
            if(config.recount == ALL) {
                count_triangles_kernel<0,1> <<< dim3(numBlocks, config.numParallelTiles), numThreadsPerBlock >>> (g_d, info);
            } else { // config.recount == AFFECTED
                count_triangles_kernel<0,0> <<< dim3(numBlocks, config.numParallelTiles), numThreadsPerBlock >>> (g_d, info);
            }
        }
...
void ktruss(COOGraph* gdir_d, COOGraph* truss_d, Info info, Config config) {
    ...
    startTimer(&timer);
    TiledCOOCSRGraph* g_d = createEmptyTiledCOOCSROnDevice(config.numNodes, config.numTiles, config.numEdges/2);
    coo2tiledcoocsrOnDevice(gdir_d, g_d);
    syncStopAndPrintElapsed(&timer, "    Convert directed COO to tiled COOCSR (not optimized)");
